...
class DL2Neo4jGraphStore(Neo4jGraphStore):
    ...
    def get_rel_map(self, subjs: List[str] | None = None, depth: int = 2, limit: int = 30):
        '''
...
gs = DL2Neo4jGraphStore(username="neo4j", password="030108chen",
...
def query():
    qe = KnowledgeGraphQueryEngine(storage_context=stc, llm=llm, verbose=True,
                                   graph_query_synthesis_prompt=gq_synthesis_prompt
                                   )
    response = qe.query(",".join(
        ["What is one of the most popular papers in the task field of Medical Image Segmentation?", "what method does it propose?"]))
    print(response)
...
if __name__ == "__main__":
    retriever = KnowledgeGraphRAGRetriever(
        ...
        graph_query_synthesis_prompt=gq_synthesis_prompt, graph_response_answer_prompt=kg_answer_prompt)
    qe = RetrieverQueryEngine.from_args(retriever=retriever, )
    response = qe.query("Tell me about the large language model llama.", )
    print(response)
